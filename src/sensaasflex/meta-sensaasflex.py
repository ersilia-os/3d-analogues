...
try:
    ...
    print('usage: meta-sensaasflex.py sdf-target-file-name sdf-source-file-name -r nb -s score_type -l x\noption -r: nb is an integer\noption -s: { source target mean }\noption -l: x is an integer')
    quit()
...
if(nbarg==6):
    if(sys.argv[4]=="-s"):
        if(sys.argv[5]=="mean" or sys.argv[5]=="source" or sys.argv[5]=="target"):
...
if(nbarg==8):
    ...
    if(sys.argv[4]=="-l"):
        loop=int(sys.argv[5])
    if(sys.argv[6]=="-s"):
        if(sys.argv[7]=="mean" or sys.argv[7]=="source" or sys.argv[7]=="target"):
...
#sensaasexe=re.sub('meta-sensaasflex\.py','',sensaasexe)
#sensaasexe="./"
...
## MODIFIED to fit the right path
whichexe='linux'
if(whichexe in platform):
    # linux
    sensaasexeflex=f"python {os.path.join(root,'sensaasflex.py')}"
    sensaasexe=f"python {os.path.join(root,'sensaas.py')}"
    """
    sensaasexeflex=sensaasexe+ "sensaasflex.py"
    ...
    #windows
    sensaasexeflex="python " + sensaasexe+ "sensaasflex.py"
...
#Database mode:
    ...
    output=os.path.join(output_folder, "catsensaas.sdf")
    outputbest=os.path.join(output_folder,"bestsensaas.sdf")
    outputmatrix=os.path.join(output_folder, "matrix-sensaas.txt")
